...
ggsave(
  filename = "productive_authors_plot.png",
...
# ggsave(
#   filename = "productive_countries_plot.png",
...
ggraph(bigram_graph2, layout = "fr") + 
  ...
  scale_edge_colour_continuous(breaks = log(c(100, 500, 3000)), labels =c(100, 500, 3000)) +
...
ggsave(
  filename = "gamma_through_time.png",
...
ggsave(
  filename = "author_network_plot.png",
...
ggsave(
  filename = "author_network_plot.pdf",
  plot = author_network_plot,
  device = "pdf",
  path = "images/",
  width = 190,
...
ggsave(
  filename = "country_networkgraph.png",
...
ggsave(
  filename = "country_mentions_plot.png",
...
<!-- Deal with accents `mutate(authkeywords_mod = stri_trans_general(authkeywords_mod, "Latin-ASCII"))` -->
...
<!--       if (ret_string != "" & !is.na(ret_string)) {  -->
<!--         ret_string <- rvest::html_text(rvest::read_html(charToRaw(s))) -->
<!--       } -->
<!--       return(ret_string) -->
<!--     } -->

<!-- Removes after copywrite symbol from abstracts -->

<!--     strip_copyright <- function(string) -->
<!--     { -->
<!--       # ret_string <- string -->
<!--       # if(!is.na(string) & str_detect(string, pattern = "©") == TRUE) { -->
<!--       #   str.pos <- str_locate(string, "©")[1] # find symbol                                                                               -->
<!--       #   ret_string <- (str_sub(string, 1, str.pos - 1)) # rm it and following text -->
<!--       # } -->
<!--       #  -->
<!--       # return(ret_string) -->
...
<!--       ret_string <- string -->
<!--       if(!is.na(ret_string)) { -->
<!--         while (str_detect(ret_string, "©") == TRUE) { -->
<!--            ret_string <- str_extract(ret_string, "^.*(?=(©))") -->
<!--         } -->
<!--       }   -->
<!--       return(ret_string) -->
...
<!-- Identifies which language abstract is -->
...
<!-- # from George -->
<!-- # Bit janky. And not sure I got all the accents but you can add to the list. -->
...
<!--   repl_string <- chartr(old = paste(names(unwanted_array), collapse = ''), new = paste(unwanted_array, collapse = ''), string) -->
